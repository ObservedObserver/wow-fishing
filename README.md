...
### `timing.max_cast_lifetime_ms`

单次甩竿最多等待多久，默认是 30000 毫秒：

```yaml
timing:
  max_cast_lifetime_ms: 30000
```

只要前面检测到咬钩声音，脚本会照常收杆；如果一直没有检测到，超过这个时长后会强制结束本轮并重新甩竿。
...
- `[slot2] ...`
  - 说明程序在第一次收杆后等待一个短延时再补一次 2 号位，或者在后续下一轮抛竿前自动补一次 2 号位道具
